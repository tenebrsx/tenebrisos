...
import MobileDashboardWeb from "../components/MobileDashboardWeb";
...
const Home = () => {
  ...
  const [activities, setActivities] = useState(() => {
    const loaded = loadFromStorage("activities", []);
  ...
  // AI & Intelligence - defer initialization until activities are loaded
  ...
  // Initialize AI engine after activities are loaded
  useEffect(() => {
    if (activities.length >= 0 && !aiEngine) {
      try {
        console.log(
          "🚀 Initializing AI engine with",
          activities.length,
          "activities",
        );
        const engine = createAILearningEngine();
        setAiEngine(engine);
      } catch (error) {
        console.error("❌ Failed to initialize AI engine:", error);
        // Create a minimal engine if initialization fails
        setAiEngine({
          generatePersonalInsights: () => [],
          getIntelligentSuggestions: () => ({ suggestedActivities: [] }),
          getCurrentUserState: () => ({}),
          getUserInsights: () => ({}),
        });
      }
  ...
  const getActivityPatterns = () => {
    ...
    const pastWeek = activitiesArray.filter((activity) => {
  ...
  const generateSuggestedActions = async () => {
    ...
    try {
      ...
      // Get user context from AI learning engine (if available)
      ...
      const activitiesArray = Array.isArray(activities) ? activities : [];
      const recentActivities = activitiesArray.slice(0, 10);
      ...
      const scoredSuggestions = baseSuggestions.map((suggestion) => {
        ...
        const hasRecentSimilar = Array.isArray(recentActivities)
        ...
        const categoryCount = Array.isArray(recentActivities)
  ...
  useEffect(() => {
    const refreshActivities = () => {
      ...
      const activitiesArray = Array.isArray(latestActivities)
        ? latestActivities
        : [];
      setActivities(activitiesArray);

      const activeActivity = activitiesArray.find((a) => a.isActive);
  ...
  useEffect(() => {
    if (aiEngine) {
      ...
      const activitiesArray = Array.isArray(activities) ? activities : [];
      if (shouldUpdate && activitiesArray.length > 0) {
            ...
            <AnimatePresence mode="wait">
              {currentActivity ? (
                ...
                >
                  ...
                  >
                    <div className="text-6xl font-mono font-bold text-accent-blue">
                    ...
                    {isRetroTheme ? (
                      ...
                      <>
                        ...
                        <MagneticButton
                          ...
                          size="lg"
                          onClick={handleStopActivity}
                          className="flex items-center space-x-2 bg-accent-green"
                        >
                          <Square size={20} />
                          <span>Complete</span>
                        </MagneticButton>
                ...
                >
                  <div className="mb-6">
                    <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-accent-blue/20 border border-white/20 flex items-center justify-center">
                      <Clock size={24} />
                    </div>
                  ...
                  <div className="flex justify-center">
                    {isRetroTheme ? (
                      ...
                      <MagneticButton
                        ...
                        size="lg"
                        onClick={() => navigate("/schedule")}
                        className="flex items-center space-x-2 bg-accent-blue"
                      >
                        <Zap size={20} />
