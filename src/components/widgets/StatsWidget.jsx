...
const StatsWidget = ({ widgetId, size = "medium", isCustomizing }) => {
  ...
  useEffect(() => {
    const calculateStats = () => {
      try {
        const activitiesData = loadFromStorage("activities", []) || [];
        const activities = Array.isArray(activitiesData) ? activitiesData : [];
        const notesData = loadFromStorage("notes", []) || [];
        const notes = Array.isArray(notesData) ? notesData : [];
        const todosData = loadFromStorage("todos", []) || [];
        ...
        const weekActivities = completedActivities.filter(
          (activity) => new Date(activity.startTime) >= weekAgo,
        );
        ...
        for (let i = 0; i < 365; i++) {
          ...
          const hasActivity = activities.some(
            (activity) =>
              new Date(activity.startTime).toDateString() === dateStr &&
              !activity.isActive,
          );
        ...
        const mostProductiveHour = Object.keys(hourlyData).reduce(
        ...
        const favoriteActivity = Object.keys(activityTypes).reduce(
          (a, b) => (activityTypes[a] > activityTypes[b] ? a : b),
          null,
        );
    ...
    switch (size) {
      ...
      case "large":
        return [
          ...
          {
            label: "This Week",
            value: formatDuration(stats.weekTime),
            icon: Calendar,
            color: "accent-red",
          },
          {
            label: "Daily Avg",
            value: formatDuration(Math.round(stats.averageDaily)),
            icon: TrendingUp,
            color: "accent-blue",
          },
        ];
    ...
    return (
      ...
      >
        <div className="flex items-center space-x-3">
          <div
            className={`w-8 h-8 rounded-lg bg-${stat.color}/20 flex items-center justify-center`}
          >
