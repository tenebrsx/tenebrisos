...
export class AILearningEngine {
  constructor() {
    ...
    this.activityHistory = Array.isArray(activities) ? activities : [];

    // Ensure other arrays are properly initialized
    const scheduleHistory = loadFromStorage("scheduleHistory", []);
    this.scheduleHistory = Array.isArray(scheduleHistory)
      ? scheduleHistory
      : [];

    const suggestionInteractions = loadFromStorage(
      "suggestionInteractions",
      [],
    );
    this.suggestionInteractions = Array.isArray(suggestionInteractions)
      ? suggestionInteractions
      : [];
  ...
  initializeLearning() {
    console.debug("🚀 AI Learning: Initializing learning engine...");

    // Ensure activityHistory is an array before processing
    if (!Array.isArray(this.activityHistory)) {
      console.warn(
        "⚠️ AI Learning: activityHistory is not an array, reinitializing...",
      );
    ...
    try {
      ...
      console.error("❌ AI Learning: Error during initialization:", error);
      // Reset activityHistory if there's an error
      this.activityHistory = [];
    }

    console.debug("🧠 AI Learning: Engine initialized with", {
